...
const stripePromise = loadStripe(
  "pk_test_51RnLFbFW8XM59bhWaYOroZ29ELB4xWqWiadqhP8CPAl3RvZL8ahBcZTdHxI94f9r0hn9IJNaO8BOGcQXrpE2SBYF00CkTXOTtd"
...
function CheckoutForm({
    ...
    const initializeData = async () => {
      try {
        ...
        if (token) {
          ...
          const name =
            res.data.name || (res.data.data && res.data.data.name) || "";
          const email =
            res.data.email || (res.data.data && res.data.data.email) || "";
          setUser({ name, email });
  ...
  const applyCoupon = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        setCouponSuccess(
          `Coupon applied! You'll save $${res.data.data.discountAmount}`
        );
        setCouponCode("");
      } else {
  ...
  useEffect(() => {
    if (selectedAddressId && addresses.length > 0 && countries.length > 0) {
      ...
      if (addr) {
        ...
        const countryId =
          countries.find((c) => c.name === addr.country)?._id || "";
    ...
    async function fetchStates() {
      try {
        ...
        if (isMounted) {
          const data = res.data;
          setAddressStates(
            data.data && data.data.states ? data.data.states : []
          );
        }
      } catch (e) {
        if (isMounted) {
    ...
    async function fetchCities() {
      try {
        ...
        if (isMounted) {
          const data = res.data;
          setAddressCities(
            data.data && data.data.cities ? data.data.cities : []
          );
        }
      } catch (e) {
        if (isMounted) {
  ...
  const handleAddressFormChange = (e) => {
    ...
    if (name === "country") {
      ...
      setAddressForm((f) => ({
        ...f,
        country: countries.find((c) => c._id === value)?.name || "",
      }));
    } else if (name === "state") {
      setAddressState(value);
      setAddressForm((f) => ({
        ...f,
        state: addressStates.find((s) => s._id === value)?.name || "",
      }));
    } else if (name === "city") {
      setAddressCity(value);
      setAddressForm((f) => ({
        ...f,
        city: addressCities.find((c) => c._id === value)?.name || "",
      }));
    } else {
      setAddressForm((f) => ({
        ...f,
  ...
  const handleSubmit = async (e) => {
    ...
    const countryCode =
      countries.find((c) => c._id === selectedCountry)?.code ||
      countryNameToCode[
        countries.find((c) => c._id === selectedCountry)?.name
      ] ||
      "";
  ...
  return (
    <>
      ...
      >
          ...
          <div className="mb-4">
            ...
            {addresses.map((addr) => (
              <div key={addr._id} className="flex items-center mb-2">
                ...
                {addr.isDefault && (
                  <span className="ml-2 text-xs text-orange-500">Default</span>
                )}
              </div>
            ))}
            <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <input
              ...
              className="w-full py-3 px-4 border rounded-lg focus:ring-2 focus:ring-orange-400"
              required
            />
          </div>
          <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div>
            ...
            <select
              ...
              className="form-select w-full py-3 px-4 border rounded-lg focus:ring-2 focus:ring-orange-400"
            >
              <option value="">Select Country</option>
            ...
            {availableCoupons.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {availableCoupons.map((coupon) => (
                  <div
                    key={coupon._id}
                    className="p-3 border border-green-200 rounded-lg bg-white"
                  >
              ...
              <p className="text-gray-500">
                No available coupons for your order total.
              </p>
            )}
          </div>
          ...
          <div className="mb-4">
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <input
                ...
                className="w-full sm:flex-1 py-2 px-3 border rounded-lg focus:ring-2 focus:ring-orange-400"
              ...
              <button
                ...
                className="w-full sm:w-auto px-4 py-2 bg-orange-500 text-white rounded-lg hover:bg-orange-600 disabled:opacity-50"
              >
              ...
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-sm text-green-600">
                    You'll save ${selectedCoupon.discountAmount}
                  </p>
                </div>
                <button
                  type="button"
        ...
        {error && (
          <div className="text-red-600 mb-2" role="status">
            {error}
          </div>
        )}
        {success && (
  ...
  useEffect(() => {
    async function fetchCountries() {
      try {
        ...
        const data = res.data;
        setCountries(
          data.data && data.data.countries ? data.data.countries : []
        );
      } catch (e) {
        setCountries([]);
      }
        ...
        <div className="container relative">
          <div className="grid lg:grid-cols-12 md:grid-cols-2 grid-cols-1 gap-6">
            ...
            <div className="lg:col-span-4">
              <div
                className="p-6 rounded-md shadow dark:shadow-gray-800"
                role="region"
              ...
              >
                ...
                <div className="mt-4 rounded-md shadow dark:shadow-gray-800">
                  {cartData.map((item, idx) => (
                    <div
                      key={idx}
                      className="p-3 flex justify-between items-center border-b border-gray-100 dark:border-gray-800"
                    >
                      <div className="flex items-center">
                        <img
                          src={
                            item.image?.startsWith("/uploads")
                              ? `https://zyqora.onrender.com${item.image}`
                              : item.image
                          }
                          alt={item.name}
                  ...
                  <div className="p-3 flex justify-between items-center border-t border-gray-100 dark:border-gray-800">
                    ...
                    <p className="font-semibold">
                      {totals.subtotal.toLocaleString("en-CA", {
                        style: "currency",
                        currency: "CAD",
                      })}
                    </p>
                  ...
                  <div className="p-3 flex justify-between items-center border-t border-gray-100 dark:border-gray-800">
                    ...
                    <p className="font-semibold">
                      {totals.tax.toLocaleString("en-CA", {
                        style: "currency",
                        currency: "CAD",
                      })}
                    </p>
                  ...
                  {selectedCoupon && (
                    <div className="p-3 flex justify-between items-center border-t border-gray-100 dark:border-gray-800">
                      ...
                      <p className="font-semibold text-green-600">
                        -
                        {selectedCoupon.discountAmount.toLocaleString("en-CA", {
                          style: "currency",
                          currency: "CAD",
                        })}
                      </p>
