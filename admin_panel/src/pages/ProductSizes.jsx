...
export default function ProductSizes() {
  ...
  const fetchProductSizes = async () => {
    ...
    try {
      ...
      const response = await productSizesAPI.getProductSizes(params);
      let data = response.data;
      if (data.data && data.data.productSizes) {
        setProductSizes(data.data.productSizes);
    ...
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-600 mt-2">Manage product-size associations</p>
        </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
        <div className="flex gap-3">
          <div className="flex-1 relative">
            <input
              type="text"
              placeholder="Search by product or size... (Press Enter to search)"
              value={searchTerm}
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="bg-white divide-y divide-slate-200">
              {productSizes.map((ps) => (
                <tr key={ps._id} className="hover:bg-slate-50 transition-colors">
        ...
        {productSizes.length === 0 && !loading && (
          <div className="text-center py-12">
            ...
            {!searchTerm && (
              <div className="mt-6">
                <button
                  onClick={() => { navigate('/product-sizes/add'); }}
