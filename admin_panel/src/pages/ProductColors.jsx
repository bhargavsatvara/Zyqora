...
export default function ProductColors() {
  ...
  const fetchProductColors = async () => {
    ...
    try {
      ...
      const response = await productColorsAPI.getProductColors(params);
      let data = response.data;
      if (data.data && data.data.productColors) {
        setProductColors(data.data.productColors);
  ...
  const handleDelete = async () => {
    ...
    setDeleteError('');
    try {
      await productColorsAPI.deleteProductColor(deleteId);
      setDeleteId(null);
      showSuccess('Product colors association deleted successfully!');
      fetchProductColors();
    } catch (err) {
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">Product Colors</h1>
          <p className="text-slate-600 mt-2">Manage product-color associations</p>
        </div>
        <button onClick={() => navigate('/product-colors/add')} className="bg-gradient-to-r from-blue-500 to-blue-600 text-white px-6 py-3 rounded-xl hover:from-blue-600 hover:to-blue-700 flex items-center gap-2 shadow-lg transition-all">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
        <div className="flex gap-3">
          <div className="flex-1 relative">
            <input
              type="text"
              placeholder="Search by product or color... (Press Enter to search)"
              value={searchTerm}
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          ...
          <table className="w-full">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Colors</th>
                <th className="px-6 py-4 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Actions</th>
              </tr>
            </thead>
            <tbody className="bg-white divide-y divide-slate-200">
              {productColors.map((pc) => (
                <tr key={pc._id} className="hover:bg-slate-50 transition-colors">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm font-medium text-slate-900">{pc.product_id?.name || '—'}</div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="text-sm text-slate-600 max-w-xs">
                      {getColorNames(pc.color_ids)}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <div className="flex items-center gap-2">
                      <button 
                        onClick={() => navigate(`/product-colors/edit/${pc._id}`)}
        ...
        {productColors.length === 0 && !loading && (
          <div className="text-center py-12">
            ...
            {!searchTerm && (
              <div className="mt-6">
                <button
                  onClick={() => { navigate('/product-colors/add'); }}
      ...
      {deleteId && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl shadow-xl border border-slate-200 w-full max-w-md p-8 relative">
            ...
            <p className="mb-4 text-slate-700">Are you sure you want to delete this product colors association? This action cannot be undone.</p>
