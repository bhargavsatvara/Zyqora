...
export default function EditSizeChart() {
  ...
  const fetchSizeChart = async () => {
    try {
      ...
      showError('Error fetching size chart');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || 'Failed to update size chart');
      }

      showSuccess('Size chart updated successfully');
      navigate('/size-charts');
    } catch (error) {
      console.error('Error updating size chart:', error);
      showError(error.message || 'Error updating size chart');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-gray-600">Update size chart information</p>
          </div>
        </div>
      </div>
